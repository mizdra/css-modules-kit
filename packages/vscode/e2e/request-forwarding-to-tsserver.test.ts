...
describe('Go to Definition for specifiers with import alias', () => {
  test('resolves to the correct .module.css file', async () => {
    await vscode.workspace.openTextDocument(aModuleCSS1Path);
    const links1 = await vscode.commands.executeCommand<vscode.DocumentLink[]>(
      'vscode.executeLinkProvider',
      aModuleCSS1Path,
    );

    assert.equal(links1.length, 1);
    assert.ok(links1[0]!.target?.toString() === bModuleCSS1Path.toString());
    assert.deepEqual(
      toObject(links1[0]!.range),
      toObject(new vscode.Range(2, 9, 2, 33)), // `@/src/dir-1/b.module.css`
    );
  });
  test('works in other projects', async () => {
    await vscode.workspace.openTextDocument(aModuleCSS2Path);
    const links2 = await vscode.commands.executeCommand<vscode.DocumentLink[]>(
      'vscode.executeLinkProvider',
      aModuleCSS2Path,
    );

    assert.equal(links2.length, 1);
    assert.ok(links2[0]!.target?.toString() === bModuleCSS2Path.toString());
    assert.deepEqual(
      toObject(links2[0]!.range),
