...
describe('createResolver', async () => {
  ...
  test('resolves a relative specifier', () => {
  ...
  test('returns undefined for a specifier whose file does not exist', () => {
    const resolve = createResolver(
      normalizeCompilerOptions({ paths: { '@/*': ['./paths1/*'] } }, iff.rootDir),
      undefined,
    );
    expect(resolve('./non-existent.module.css', { request })).toBe(undefined);
    expect(resolve('@/non-existent.module.css', { request })).toBe(undefined);
  });
  test('resolves a specifier with `paths`', () => {
    const resolve = createResolver(
      normalizeCompilerOptions({ paths: { '@/*': ['./paths1/*'] } }, iff.rootDir),
      undefined,
    );
  ...
  test('tries the substitutions of `paths` in order', () => {
    const resolve = createResolver(
      normalizeCompilerOptions({ paths: { '@/*': ['./paths1/*', './paths2/*'] } }, iff.rootDir),
      undefined,
    );
    expect(resolve('@/a.module.css', { request })).toBe(iff.paths['paths1/a.module.css']);
    expect(resolve('@/b.module.css', { request })).toBe(iff.paths['paths2/b.module.css']);
  });
  test('resolves the substitutions of `paths` relative to `baseUrl`', () => {
    const resolve = createResolver(
  ...
  test('resolves a specifier with the `imports` field of package.json', () => {
    const resolve = createResolver(
      normalizeCompilerOptions(
        {
          moduleResolution: 'bundler',
  ...
  test('resolves a bare specifier relative to `baseUrl`', () => {
    const resolve = createResolver(
      normalizeCompilerOptions(
        {
          baseUrl: join(iff.rootDir, 'dir'),
  ...
  test('resolves a specifier of a file in a package', () => {
    const resolve = createResolver(normalizeCompilerOptions({}, iff.rootDir), undefined);
  ...
  // NOTE: css-modules-kit does not support the `~` prefix of css-loader.
  test('returns undefined for a specifier with the `~` prefix', () => {
    const resolve = createResolver(normalizeCompilerOptions({}, iff.rootDir), undefined);
    expect(resolve('~package/a.module.css', { request })).toBe(undefined);
  });
  test('returns undefined for a URL specifier even if `paths` matches it', () => {
    const resolve = createResolver(
      normalizeCompilerOptions(
        {
          paths: {
